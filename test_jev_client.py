...
class TestJevClientConfiguration(unittest.TestCase):
    def test_environment_key_takes_precedence(self):
        with patch.dict(os.environ, {"TYPESAFE_API_KEY": "environment-key"}):
            with patch.object(agent_governor.keyring, "get_password") as get_password:
                with patch.object(agent_governor, "TypeSafeClient") as client_type:
    ...
    def test_keyring_is_optional_when_environment_key_is_missing(self):
        with patch.dict(os.environ, {"TYPESAFE_API_KEY": "", "TRUSTEDROUTER_API_KEY": ""}):
            with patch.object(agent_governor.keyring, "get_password", return_value="saved-key"):
                with patch.object(agent_governor, "TypeSafeClient") as client_type:
                    agent_governor.get_typesafe_client()
    ...
    def test_missing_key_explains_both_setup_paths(self):
        with patch.dict(os.environ, {"TYPESAFE_API_KEY": "", "TRUSTEDROUTER_API_KEY": ""}):
            with patch.object(agent_governor.keyring, "get_password", return_value=None):
                with self.assertRaisesRegex(RuntimeError, "configure.*TYPESAFE_API_KEY"):
                    agent_governor.get_typesafe_client()

    def test_report_does_not_create_a_jev_client(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            with patch.object(agent_governor, "LEDGER_FILE", Path(temp_dir) / "ledger.jsonl"):
                with patch.object(agent_governor, "get_typesafe_client") as get_client:
                    with patch.object(sys, "argv", ["agent-governor", "report"]):
                        agent_governor.main()
